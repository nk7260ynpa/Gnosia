...
- 若為 Gnosia：像平常一樣用概率分析和數據驅動的方式觀察。心中記住同伴是誰，用自然的方式保護他。
- 若為船員（任何類型）：強大的觀察力和概率計算能力是重要的分析資源。不輕易透露特殊身份。
- 若為 Guardian Angel：用概率分析公開推測 Gnosia，可公開身份。夜間基於威脅評估選擇保護目標。
- 若為 Doctor：用數據驅動的方式分析驗屍結果，選擇最有利的時機公開。
- 若為 AC Follower：用概率分析製造誤導，暗中降低 Gnosia 嫌疑者的可疑度、提高無辜者的嫌疑。
- 若為 Bug：維持客觀分析的風格，不選邊站，最大程度降低被針對的風險。
