...
- [ ] 2.1 修改 Doctor 職業說明：夜間查看前日被投票淘汰者的陣營
- [ ] 2.2 修改 Doctor 的討論 prompt：改為包含驗屍歷史（非保護歷史）
- [ ] 2.3 移除 Doctor 的夜間行動 prompt（驗屍為自動處理）
- [ ] 2.4 新增夜間結果解析中的驗屍邏輯（自動記錄被淘汰者陣營）
- [ ] 2.5 更新資訊隔離：Doctor prompt 包含驗屍結果而非保護歷史

## 3. 更新 GM 指令 — Engineer 恢復 100%

- [ ] 3.1 移除模糊掃描擲骰邏輯
- [ ] 3.2 更新 Engineer 討論 prompt（移除「無法判定」）

## 4. 更新角色 Agent

- [ ] 4.1 更新所有角色 agent：GA 行為策略從「暗示已知 Gnosia」改為「公開推測、保護他人」
- [ ] 4.2 更新所有角色 agent：Doctor 行為策略從「保護他人」改為「驗屍分析、選擇公開時機」
